...
export class TCS3000 extends BaseDispenser {
	...
	getProductIDBytes(productId?: number) {
		...
		const productIdHighByte = (parsedProductId >> 8) & 0xff;
		const productIdLowByte = parsedProductId & 0xff;
	...
	async sendPreset(quantity: number, productId?: number) {
		let crc = 0;
		const buffer_array = [];

		const [productIdHighByte, productIdLowByte] = this.getProductIDBytes(productId);
